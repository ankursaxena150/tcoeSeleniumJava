...
public class InternetHomePage extends PageObject {
    ...
    @FindBy(xpath = "//div[@class='manifest']")
    ...
    public boolean isLoaded(String name) {
        try {
            getDriver().findElement(By.xpath("//h4[text()='"+SEARCH_PMR_FORM_NAME+"']"));
        } catch (NoSuchElementException e) {
            logger.info("User " + name + " has not been authorized");
//            child.get().log(Status.INFO, "User " + name + " has not been authorized");
            return false;
        }
        return true;
    }
